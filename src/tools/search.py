...
Uses the duckduckgo-search library when available, falls back to HTTP API.
"""
...
def _search_via_fallback(query: str, max_results: int = 10) -> List[Dict[str, Any]]:
    ...
    # Parse results from HTML (best-effort regex extraction)
    ...
    for href, title, snippet in blocks[:max_results]:
        ...
        # DuckDuckGo HTML wraps URLs in a redirect
        if "uddg=" in href:
...
class SearchTool(BaseTool):
    ...
    description = (
        ...
        "title, URL, and snippet for each. No tracking, no API key needed."
    )
    parameters = {
        "query": {
            "type": "string",
        ...
        "region": {
            "type": "string",
            "description": "Region code for localized results (e.g. 'us-en', 'wt-wt' for worldwide).",
            "required": False,
        },
    }
